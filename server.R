...
function(input, output, session) {
  ...
  output$show_list <- renderReactable({
    reactable(dat_show_list(),
              ...
              defaultSorted = list('Venue' = 'asc'),
              filterable=T, searchable = T, fullWidth = T, compact = T, pagination = F, striped=T)
  })
  ...
  output$timeline <- renderPlotly({
    ...
    ggplotly(
      ggplot(plotdata, aes(x = Year, y = Month)) +
        geom_tile(aes(fill = Shows, width=.8, height=.8, color='white')) + 
        scale_y_discrete(limits = rev) + 
